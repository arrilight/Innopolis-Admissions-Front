...
import { LoadTestsPoolFail, LoadTestsPoolSuccess, TestsAction, TestsActionTypes } from './tests.actions';
...
const reducers: Record<TestsActionTypes, ActionReducer<TestsState, TestsAction>> = {
...
export function loadTestsPoolSuccess(state: TestsState, { payload: { tests } }: LoadTestsPoolSuccess): TestsState {
  return testsApapter.addMany(tests, { ...state, isLoading: false });
}

export function loadTestsPoolFail(state: TestsState, { payload: { error } }: LoadTestsPoolFail): TestsState {
  return { ...state, isLoading: false };
}

export function reducer(state = testsInitialState, action: TestsAction): TestsState {
